...
public class Circle : Shape
{
    ...
    internal override void HandlePointerMove(PointerEventArgs eventArgs)
    {
        ...
        {
            case EditMode.Add:
                double askedRadius = Math.Max(Math.Abs(point.X - Cx), Math.Abs(point.Y - Cy));
            ...
            case EditMode.MoveAnchor:
                ...
                double targetRadius = MathHelper.RadiusFromPointAndCenter(Cx, Cy, point.X, point.Y);
                R = GetMaxRadius(SvgEditor.ImageBoundingBox, new Coord<double>(Cx, Cy), targetRadius).Round();

                if (R < 1)
                    R = 1; //Mindestgröße des Kreises
    ...
    internal override async Task HandlePointerUp(PointerEventArgs eventArgs)
    {
        ...
        {
            if (R == 0)
                R = GetMaxRadius(SvgEditor.ImageBoundingBox, new Coord<double>(Cx, Cy), 15); //Wenn Radius 0 ist, wurde der Kreis nur durch ein Klicken erzeugt, also wird er auf 15 gesetzt
    ...
    private static double GetMaxRadius(BoundingBox outerBox, Coord<double> centerCoord, double askedRadius)
    {
